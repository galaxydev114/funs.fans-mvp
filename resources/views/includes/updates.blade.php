...
<div class="card-footer bg-white border-top-0">
    <h4>
			@php
			$likeActive = Auth::check() && Auth::user()->likes()->where('updates_id',$response->id)->where('status','1')->count();
			...
			<a href="javascript:void(0);" class="btnLike @if($likeActive != 0 )active @endif {{$buttonLike}} text-muted mr-2" data-id="{{$response->id}}">
				<i class="@if($likeActive != 0)fas @else far @endif fa-heart"></i> <small><strong class="countLikes">{{Helper::formatNumber($response->likes()->count())}}</strong></small>
			</a>
