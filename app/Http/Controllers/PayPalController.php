      ...
      $earningNetAdmin = number_format($processorFees - $earningNetUser, 2);
	    ...
	    if ($verified) {
	          ...
	          // Check outh POST variable and insert in DB
						$verifiedTxnId = Subscriptions::where('txn_id', $txn_id)->first();
        ...
        $sql->txn_id  = $txn_id;
        $sql->user_id = $data['id'];
        $sql->subscriber = $data['subscriber'];
        $sql->name = $data['name'];
        $sql->ends_at = Carbon::now()->add(1, 'month');
        $sql->price   = $data['amount'];
        $sql->earning_net_user  = $earningNetUser;
        $sql->earning_net_admin = $earningNetAdmin;
